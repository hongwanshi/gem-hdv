...
evi_fleetGen <- function(evi,fleet_size,pev_weights,pref_weights,home_weights,work_weights,public_weights,vmt_weights=NULL) {
  ...
  all_perms[, pev_type := factor(pev_type, c("PHEV20","PHEV50","BEV600","BEV800"))]
  ...
  while( (i<=4) & (nrow(fleet[is.na(unique_vid)]) > 0) ) {
    ...
    if( nrow(row_to_match[pev_type=="BEV100"])>0 ) {
      print(paste0("    EVI Fleet Generation: converting ",as.character(nrow(row_to_match[pev_type=="BEV600"]))," BEV600s to BEV800s."))
      row_to_match[pev_type=="BEV600",pev_type:="BEV800"]
    }
    if( nrow(row_to_match[pev_type=="PHEV50"])>0 ) {
      print(paste0("    EVI Fleet Generation: converting ",as.character(nrow(row_to_match[pev_type=="PHEV50"]))," PHEV50s to BE600s."))
      row_to_match[pev_type=="PHEV50",pev_type:="BEV600"]
    }
