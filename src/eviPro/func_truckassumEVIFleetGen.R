...
truckevi_fleetGen <- function(evi,fleet_size,pev_weights,pref_weights,home_weights,work_weights,public_weights,vmt_weights=NULL) {
  ...
  all_perms[, pev_type := factor(pev_type, c("PHEV100","PHEV200","BEV600","BEV800"))]
  ...
  while( (i<=4) & (nrow(fleet[is.na(unique_vid)]) > 0) ) {
    ...
    if( nrow(row_to_match[pev_type=="PHEV25"])>0 ) {
      print(paste0("    EVI Fleet Generation: converting ",as.character(nrow(row_to_match[pev_type=="PHEV25"]))," PHEV25s to PHEV50s"))
      row_to_match[pev_type=="PHEV25",pev_type:="PHEV50"]
    }
