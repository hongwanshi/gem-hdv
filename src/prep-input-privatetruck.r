...
prep.inputs.personal.charging.hdv2 <- function(exper.row,param.names,common.inputs,inputs.mobility){
  ###########################################################
  # Setup variables and environment
  ###########################################################
  workingDir <- pp(gem.raw.inputs,"NREL-EVI-Pro-Preprocessed-Profiles")

  truckDir <- pp(gem.raw.inputs,"trucks/hevi-pro/")
  ...
  if(truckfractionSAEVs < 1){
    ...
    fleet.sizes <- inputs.truck$parameters$truckdemandUnscaled[,list(n.vehs=round(sum(value)/length(days)/2*truckelectrificationPenetration*(1-truckfractionSAEVs),0)),by='rmob']
      ...
      for(season in u(toupper(dates$seasons))){
        ...
        for(weekday.type in u(dates$day.types)){
          ...
          if(file.exists(cache.file)){
            ...
            evi_fleet[,start_time_hr:=start_time*24+1]
            evi_fleet[,end_time_prk_hr:=end_time_prk*24+1]
            evi_fleet[,end_time_chg_hr:=end_time_chg*24+1]
    ...
    truckpersonalEVChargeEnergyUB1 = data.table(read.csv(pp(truckDir,"HEVILOAD_ChargeEnergyUB.csv")),key="rmob")
    truckpersonalEVChargeEnergyLB1 = data.table(read.csv(pp(truckDir,"HEVILOAD_ChargeEnergyLB.csv")),key="rmob")
    truckpersonalEVChargePowerUB1 =  data.table(read.csv(pp(truckDir,"HEVILOAD_ChargePowerUB.csv")))
    truckpersonalEVChargePowerLB1 =  data.table(read.csv(pp(truckDir,"HEVILOAD_ChargePowerLB.csv")))
    truckpersonalEVChargeEnergyUB1[,value := (1-truckfractionSAEVs)*cumsum(value), by = key(truckpersonalEVChargeEnergyUB1)]
    truckpersonalEVChargeEnergyLB1[,value := (1-truckfractionSAEVs)*cumsum(value), by = key(truckpersonalEVChargeEnergyLB1)]
    truckpersonalEVChargePowerUB1[,value:=(1-truckfractionSAEVs)*value]
    truckpersonalEVChargePowerLB1[,value:=(1-truckfractionSAEVs)*value]
    inputs$parameters$truckpersonalEVChargeEnergyUB <- truckpersonalEVChargeEnergyUB1
    inputs$parameters$truckpersonalEVChargeEnergyLB <- truckpersonalEVChargeEnergyLB1
    inputs$parameters$truckpersonalEVChargePowerUB <-truckpersonalEVChargePowerUB1
    inputs$parameters$truckpersonalEVChargePowerLB <- truckpersonalEVChargePowerLB1
    inputs$parameters$truckpersonalEVFleetSize <- all.fleets[,.(rmob,value=0.5*n.veh,type=name)]
    ...
    inputs$parameters$truckpersonalEVChargers <- all.chargers[,.(rmob,value=0.5*n.ch,type=dest_type,level=dest_chg_level)]
