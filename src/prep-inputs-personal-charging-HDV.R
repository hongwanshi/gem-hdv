...
prep.inputs.personal.charging.hdv <- function(exper.row,param.names,common.inputs,inputs.mobility){
  ...
  if(truckfractionSAEVs < 1){
    ...
    fleet.sizes <- inputs.truck$parameters$truckdemandUnscaled[,list(n.vehs=round(sum(value)/length(days)/truckelectrificationPenetration*(1-truckfractionSAEVs)*2,0)),by='rmob']
