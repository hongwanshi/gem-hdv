...
plots.truck<- function(exper,all.inputs,res,plots.dir){
  ...
  for(run.i in u(vehs$run)){
    if(TRUE){
     ...
     # if(exper$runs[run.i]$fractionSAEVs<1.0)to.plot <- disag.the.private.load(to.plot,all.inputs[[run.i]]$parameters$personalEVUnmanagedLoads)
      to.plot[,tl.ordered:=ifelse(substr(tl,1,2)=='tL',pp('a',tl),tl)]
      to.plot[,col:=getPalette(tl)[match(tl.ordered,u(tl.ordered))]]
  ...
  costs <- join.on(join.on(res[['rmob']],res[['rmob-t']][,.(demandChargeCost=sum(truckdemandChargeCost),vehicleMaintCost=sum(truckvehicleMaintCost)),by=c('run','rmob')],c('run','rmob'),c('run','rmob'),c('demandChargeCost','vehicleMaintCost')),inputs$sets$rmobtor,'rmob','rmob')
  ...
  vmt <- join.on(res[['rmob-t-tb-td']],res[['rmob-td']],c('td','rmob','run'),c('td','rmob','run'),'trucktravelDistance')
  vmt <- join.on(vmt,res[['rmob-t-td']],c('td','t','rmob','run'),c('td','t','rmob','run'),'tspeed')
  vmt[,vmt:=truckvehiclesMoving*trucktravelDistance]
  vmt[,pmt:=truckdemandAllocated*trucktravelDistance]
  fleet <- res[['rmob-tb']]
  vmt.by.region <- join.on(join.on(fleet,vmt[,.(vmt=sum(vmt),pmt=sum(pmt)),by=c('rmob','tb','run')],c('rmob','tb','run'),c('rmob','tb','run')),inputs$parameters$truckvehicleLifetime[,.(value=mean(value)),by=c('tb','rmob')],c('rmob','tb'),c('rmob','tb'),'value','assumed.lifetime.')
  n.days <- max(vmt$t)/24
  vmt.by.region[,daily.vmt.per.vehicle:=vmt/truckfleetSize/n.days]
  vmt.by.region[,daily.pmt:=pmt/n.days]
  vmt.by.region[,battery.level:=gsub('tb|tb0|tb00','',tb)]
...
#  personal.evs <- rbindlist(lapply(1:length(all.inputs),function(i){ all.inputs[[i]]$parameters$personalEVFleetSize[,.(value=sum(value),run=i),by='type'] }))[,.(run,tb=pp("Private_",type),value)]
#  personal.evs <- join.on(personal.evs,run.params,'run','run')
#  personal.evs[,batt.kwh:=unlist(lapply(str_split(tb,"EV"),function(ll){ as.numeric(ll[2])}))*personal.ev.conversion.eff]
  
  to.plot.fleet <- rbindlist(list(by.r[group=='Fleet',.(value=sum(truckfleetSize),bplus=pp('SAEV_BEV',substr(tb,3,nchar(tb)))),by=c('run',param.names,'tb')]),fill=T)
  not.needed <- to.plot.fleet[,sum(value),by='tb'][V1<=1]$tb
  to.plot.fleet <- to.plot.fleet[!tb%in%not.needed]
  to.plot.fleet[,variable:=ifelse(is.na(bplus),tb,bplus)]
  to.plot.fleet[,metric:='Fleet Size']
  to.plot.fleet[,col:=getPalette(tb)[match(to.plot.fleet$tb,u(to.plot.fleet$tb))]]
  to.plot.fleet <- join.on(to.plot.fleet,res[['tb']],c('run','tb'),c('run','tb'))
  to.plot.fleet[is.na(truckbatteryCapacity),':='(truckbatteryCapacity=batt.kwh,truckconversionEfficiency=personal.ev.conversion.eff)]
  to.plot.saev.hours <- streval(pp('vehs[,.(Charging=sum(',pp('truckvehiclesCharging.',all.inputs[[1]]$sets$tl,collapse='+'),')/n.days.in.run,Moving=sum(',pp('truckvehiclesMoving.',str_replace_all(all.inputs[[1]]$sets$td,"-","."),collapse='+'),')/n.days.in.run,Idle=sum(truckvehiclesIdle)/n.days.in.run),by="run"]'))
  ...
  person.trips <- res[['rmob-t-tb-td']][,.(personTripsPerDay=sum(truckdemandAllocated)/n.days.in.run),by=c('run','tb')]
  to.plot.trips.per.saev <- join.on(to.plot.fleet[!is.na(bplus)],person.trips,c('run','tb'),c('run','tb'))
 ...
 # personal.chargers <- join.on(personal.chargers,run.params,'run','run')
  to.plot.chargers <- by.r[group=='Chargers',.(value=sum(trucknumChargers)),by=c('run',param.names,'tl')]
  to.plot.chargers[,level.kw:=as.numeric(substr(tl,3,nchar(tl)))]
  to.plot.chargers[,variable:=pp('SAEV_Chgr_',ifelse(level.kw<=20,'AC','DC'))]
  to.plot.chargers <- rbindlist(list(to.plot.chargers),fill=T)
  to.plot.chargers <- to.plot.chargers[,.(value=sum(value),tl=tl[1]),by=c('run','variable',param.names)]
  to.plot.chargers[,metric:='# Chargers']
  to.plot.chargers[is.na(tl),tl:=variable]
  to.plot.chargers[,tl.ordered:=ifelse(substr(tl,1,2)=='tL',pp('a',tl),tl)]
  to.plot.chargers[,col:=getPalette(tl)[match(tl.ordered,u(tl.ordered))]]
  
  to.plot.ch <- rbindlist(list(veh.ch),fill=T,use.names=T)
  to.plot.ch <- join.on(to.plot.ch,to.plot.ch[,.(gw=sum(gw.charging,na.rm=T)),by=c('run','t')][,.(peak.t=t[which.max(gw)]),by='run'],'run','run')
  to.plot.peak.ch <- to.plot.ch[t==peak.t,.(value=sum(gw.charging,na.rm=T)),by=c('run','tl',param.names)]
  to.plot.peak.ch[,level.kw:=as.numeric(substr(tl,3,nchar(tl)))]
  to.plot.peak.ch[,variable:=ifelse(tl=='Private EVs','Private_EV_Load',pp('SAEV_Load_',ifelse(level.kw<=20,'AC','DC')))]
  to.plot.peak.ch <- to.plot.peak.ch[,.(value=sum(value),tl=tl[1],level.kw=level.kw[1]),by=c('run',param.names,'variable')]
  to.plot.peak.ch[,metric:='Peak Load']
  to.plot.peak.ch[,tl.ordered:=ifelse(substr(tl,1,2)=='tL',pp('a',tl),tl)]
  to.plot.peak.ch[,col:=getPalette(tl)[match(tl.ordered,u(tl.ordered))]]
...
#  veh.cost <- personal.evs[,.(n=sum(value)),by=c('run','batt.kwh')][,.(privateFleetCost=sum(n*(vehPerDayCost + vehCapCost*veh.amort.ratio + batt.kwh*inputs$parameters$batteryCapitalCost$value*batt.amort.ratio + vehPerMileCosts*12e3/365))),by='run']
  ...
  costs[,totalFleetCost:=vehicleMaintCost/n.days.in.run+truckfleetCost]
  ...
  cost.key <- data.table(variable=c('infrastructureCost','totalFleetCost','totalEnergyCost'),cost.key=c('SAEV Infrastructure','SAEV Fleet','Energy'))
  ...
  # Emissions
  saev.lifetimes <- vmt.by.region[,.(truckvehicleLifetime=truckvehicleLifetime[1],truckbatteryLifetime=truckbatteryLifetime[1]),by=c('run','tb')]
  #saev.lifetimes[,veh.amort.ratio:=dailyDiscount*(1+dailyDiscount)^(truckvehicleLifetime*365) / ((1+dailyDiscount)^(truckvehicleLifetime*365) - 1)]
  #saev.lifetimes[,batt.amort.ratio:=dailyDiscount*(1+dailyDiscount)^(truckbatteryLifetime*365) / ((1+dailyDiscount)^(truckbatteryLifetime*365) - 1)]
  ...
  veh.manuf.em <- to.plot.fleet[,.(n=sum(value),truckbatteryCapacity=truckbatteryCapacity[1]),by=c('run','tb')]
  veh.manuf.em <- join.on(veh.manuf.em,saev.lifetimes,c('run','tb'),c('run','tb'))
  ...
  #veh.manuf.em[,value:=n*7.1*veh.amort.ratio+n*0.11*truckbatteryCapacity*batt.amort.ratio] # 7.1 and 0.11 are tons per vehicle
 ...
 # to.plot.em[,value:=value*weekday.to.year.factor]
  ...
  # Charging profiles
  to.plot.ch.agg <- to.plot.ch[,.(gwh=sum(gw.charging,na.rm=T)),by=c('run',param.names,'charger.level','tl','t')]
 # to.plot.ch.agg <- disag.the.private.load(to.plot.ch.agg,inputs$parameters$personalEVUnmanagedLoads)
  to.plot.ch.agg[,tl.ordered:=ifelse(substr(tl,1,2)=='tL',pp('a',tl),tl)]
  to.plot.ch.agg[,col:=getPalette(tl)[match(tl.ordered,u(tl.ordered))]]
  ...
  if(length(param.names)==1){
    ...
    for(param.ind.i in 1:nrow(param.inds)){
      ...
      for(comb.i in 1:nrow(param.combs)){
        ...
        energy.by.r.l <- res[['rmob-t-tb-tl']][run%in%u(by.r.sub$run),.(energyCharged=sum(energyCharged)),by=c('run','l','rmob')]
  ...
  if(length(param.names)==1){
    # Vehicle allocations
    to.plot <- data.table(melt(vehs,id.vars=c('tb','rmob','t',names(exper$runs))))
    to.plot <- to.plot[,.(value=sum(value)),by=c('tb','t',names(exper$runs),'variable')]
    
    to.plot[,vehicle.activity:=gsub('\\.tL0|\\.tL|\\.tL00','(',gsub('truckvehiclesCharging','Charging ',variable))]
    to.plot[,vehicle.activity:=ifelse(grepl('Charging',vehicle.activity),paste(vehicle.activity,'kW)'),vehicle.activity)]
    to.plot[,vehicle.activity:=gsub('\\.','-',gsub('\\.td','(',gsub('vehiclesMoving','Moving ',vehicle.activity)))]
    to.plot[,vehicle.activity:=ifelse(grepl('Moving',vehicle.activity),paste(vehicle.activity,' mi)'),vehicle.activity)]
    to.plot[,vehicle.activity:=ifelse(grepl('run',vehicle.activity),'Moving (private)',ifelse(grepl('vehiclesIdle',vehicle.activity),'Idling',vehicle.activity))]
    to.plot$vehicle.activity <- factor(to.plot$vehicle.activity,levels=unique(to.plot$vehicle.activity)[unique(mixedorder(to.plot$vehicle.activity))])
    
#     streval(pp('setkey(to.plot,variable,vehicle.activity,t,tb,',param.names,')'))
...
#     
    ...
    # Charging load
    streval(pp('setkey(veh.ch,tl,rmob,t,',param.names,')'))
    to.plot <- veh.ch[,.(gw.charging=sum(gw.charging)),by=c('tl','t',param.names)]
    to.plot[,charger.level:=gsub('tL|tL0|tL00','',tl)]
   ...
   # streval(pp('setkey(personal.ev.ch,tl,rmob,t,',param.names,')'))
    # to.plot.personal <- personal.ev.ch[,.(gw.charging=sum(gw.charging)),by=c('tl','charger.level','t',param.names)]
    ...
    # Energy balance
    streval(pp('setkey(en,tb,rmob,t,',param.names,')'))
    to.plot <- en[,.(soc=sum(soc)),by=c('tb','battery.level','t',param.names)]
    ...
    to.plot[,var.clean:=ifelse(grepl('tL',variable),paste('Chgr',variable,'kW'),paste('Bat',variable,'mi'))]
    to.plot[,var.clean:=gsub(' tL| tL0| tb| tb0 |tb00 |tL00',' ',var.clean)]
    ...
    # Costs
    to.plot <- data.table(melt(costs,measure.vars=c('demandChargeCost','vehicleMaintCost','infrastructureCost','truckfleetCost','energyCost'),id.vars=c('r',param.names)))[,.(value=sum(value)),by=c('variable',param.names)]
    cost.key <- data.table(variable=c('demandChargeCost','vehicleMaintCost','infrastructureCost','truckfleetCost','energyCost'),cost=c('Demand Charges','Maintenance','Infrastructure','Fleet Capital','Energy'))
    ...
    speed <- copy(inputs$parameters$tspeed)[,t:=as.numeric(substr(t,2,nchar(as.character(t))))]
    vmt2 <- join.on(vmt,speed,c('t','td','rmob'),c('t','td','rmob'),'value')
    vmt2[,tt:=trucktravelDistance*value]
    to.plot <- join.on(to.plot,vmt2[,.(vmt=sum(vmt),t=sum(tt)),by=param.names],param.names,param.names,c('vmt','t'))
    to.plot <- join.on(to.plot,fleet[,.(truckfleetSize=sum(truckfleetSize)),by=param.names],param.names,param.names,'truckfleetSize')
    ...
    # p <- ggplot(personal.ev.ch[,.(value=sum(gw.charging)),by=c('t','tl',param.names)],aes(x=t,y=value,colour=l))+geom_line()
    # streval(pp('p <- p + facet_wrap(~',param.names,')'))
    ...
    # 
    # to.plot <- veh.ch[,.(gw.charging=sum(gw.charging)),by=c('tl','t',param.names)]
    # to.plot <- personal.ev.ch[,.(gw.charging=sum(gw.charging)),by=c('tl','t',param.names)]
    # to.plot <- rbindlist(list(to.plot,to.plot),fill=T,use.names=T)
    # p <- ggplot(to.plot[,.(value=sum(gw.charging)),by=c('t','tl',param.names)],aes(x=t,y=value,colour=l))+geom_line()
    # streval(pp('p <- p + facet_wrap(~',param.names,')'))
