...
getPalette = function(vals){
	...
	if(any(u(vals)=='b075',na.rm=TRUE)|any(u(vals)=='75 kWh',na.rm=TRUE)){
		...
		ntevs <- sum(substr(u(vals),1,1)=='t')
		nbevs <- sum(substr(u(vals),1,1)=='b')
		nprivate <- ncol-nsaevs-ntevs-nbevs
		c(rev(colorRampPalette(brewer.pal(nsaevs, "Purples"))(nsaevs)),
		  rev(colorRampPalette(brewer.pal(ntevs, "Reds"))(ntevs)),
		  rev(colorRampPalette(brewer.pal(nbevs, "Blues"))(nbevs)),
...
run.all <- function(exper,all.inputs,res,plots.dir) {
  ...
  run.i = 5
	...
	for(run.i in u(lightduty.prepped$vehs$run)) {
	  ...
	  vehs <- lightduty.prepped$vehs
	  veh.ch<-lightduty.prepped$veh.ch
	  personal.ev.ch<-lightduty.prepped$personal.ev.ch
	  human.ev.ch<-lightduty.prepped$human.ev.ch
	  en<-lightduty.prepped$en
	  by.r<-lightduty.prepped$by.r
	  vehs <- heavyduty.prepped$trcks
	  veh.ch<-heavyduty.prepped$trck.ch
	  personal.ev.ch<-heavyduty.prepped$personal.trck.ch
	  human.ev.ch<-heavyduty.prepped$human.trck.ch
	  en<-heavyduty.prepped$en
	  by.r<-heavyduty.prepped$by.r
	  
		plot.lightduty.all(run.i,plots.dir,lightduty.prepped$vehs,exper,all.inputs,lightduty.prepped$veh.ch,lightduty.prepped$personal.ev.ch,lightduty.prepped$human.ev.ch,lightduty.prepped$en,lightduty.prepped$by.r,day.axis.breaks)
		plot.heavyduty.all(run.i,plots.dir,heavyduty.prepped$trcks,exper,all.inputs,heavyduty.prepped$trck.ch,heavyduty.prepped$personal.trck.ch,heavyduty.prepped$human.trck.ch,heavyduty.prepped$en.trck,heavyduty.prepped$by.r.trck,day.axis.breaks)
  ...
  by.r.bike<-bike.prepped$by.r.bike
  bike.ch<-bike.prepped$bike.ch
  vmt.by.region.bike<-bike.prepped$vmt.by.region.bike
  by.r<-lightduty.prepped$by.r
  by.r.trck<-heavyduty.prepped$by.r.trck
  veh.ch<-lightduty.prepped$veh.ch
  trck.ch<-heavyduty.prepped$trck.ch
  personal.ev.ch<-lightduty.prepped$personal.ev.ch
  costs <- costs.prepped
  vmt.by.region<-lightduty.prepped$vmt.by.region
  vmt.by.region.trck<-heavyduty.prepped$vmt.by.region.trck
  geners<-generators.prepped$geners
  vehs<-lightduty.prepped$vehs
  personal.trck.ch<-heavyduty.prepped$personal.trck.ch
  human.trck.ch<-heavyduty.prepped$human.trck.ch
	all.factors.prepped <- prepData.factor.all(all.inputs,inputs,res,lightduty.prepped$by.r,heavyduty.prepped$by.r.trck,lightduty.prepped$veh.ch,heavyduty.prepped$trck.ch,lightduty.prepped$personal.ev.ch,heavyduty.prepped$personal.trck.ch,lightduty.prepped$human.ev.ch,heavyduty.prepped$human.trck.ch,costs.prepped,lightduty.prepped$vmt.by.region,heavyduty.prepped$vmt.by.region.trck,generators.prepped$geners,lightduty.prepped$vehs,bike.prepped$by.r.bike,bike.prepped$bike.ch,bike.prepped$vmt.by.region.bike)
...
prepData.all.lightduty <- function(exper,inputs,res,plots.dir) {
	...
	personal.ev.ch[,l:='Private automated EVs']
	personal.ev.ch[,charger.level:='Private automated EVs']
	
	# Private Vehicle Charging
	human.ev.ch <- res[['rmob-t']][,.(t,rmob,gw.charging=humanEVPower/1e6,run)]
	human.ev.ch[,l:='Private human-driven EVs']
	human.ev.ch[,charger.level:='Private human-driven EVs']
	...
	output <- list('veh.mv'=veh.mv,'d.dot'=d.dot,'l.dot'=l.dot,'veh.ch'=veh.ch,'vehs'=vehs,'veh.ch'=veh.ch,'personal.ev.ch'=personal.ev.ch,'human.ev.ch'=human.ev.ch,'en'=en,'batt'=batt,'by.r'=by.r,'vmt'=vmt,'vmt.by.region'=vmt.by.region)
	return(output)
}
...
prepData.all.heavyduty <- function(exper,inputs,res,plots.dir) {
	...
	trck.ch[,gw.charging:=kw*value/(0.3*1e7)]
	...
	# Private Vehicle Charging
	personal.trck.ch <- res[['rmob-t']][,.(t,rmob,gw.charging=truckpersonalEVPower/1e6,run)]
	personal.trck.ch[,tl:='Private automated electric trucks']
	personal.trck.ch[,charger.level:='Private automated electric trucks']
	
	# Private Vehicle Charging
	human.trck.ch <- res[['rmob-t']][,.(t,rmob,gw.charging=truckhumanEVPower/1e6,run)]
	human.trck.ch[,tl:='Private human-driven electric trucks']
	human.trck.ch[,charger.level:='Private human-driven electric trucks']
	...
	output <- list('trck.mv'=trck.mv,'td.dot'=td.dot,'tl.dot'=tl.dot,'trck.ch'=trck.ch,'trcks'=trcks,'personal.trck.ch'=personal.trck.ch,'human.trck.ch'=human.trck.ch,'en.trck'=en.trck,'batt.trck'=batt.trck,'by.r.trck'=by.r.trck,'vmt.trck'=vmt.trck,'vmt.by.region.trck'=vmt.by.region.trck)
	return(output)
}
...
prepData.costs <- function(res,gen.cost,inputs) {
	# Cost
	costs <- join.on(join.on(res[['rmob']],res[['rmob-t']][,.(demandChargeCost=sum(demandChargeCost),vehicleMaintCost=sum(vehicleMaintCost),truckvehicleMaintCost=sum(truckvehicleMaintCost),bikevehicleMaintCost=sum(bikevehicleMaintCost)),by=c('run','rmob')],c('run','rmob'),c('run','rmob'),c('demandChargeCost','vehicleMaintCost','truckvehicleMaintCost','bikevehicleMaintCost')),inputs$sets$rmobtor,'rmob','rmob')
...
prepData.factor.all <- function(all.inputs,inputs,res,by.r,by.r.trck,veh.ch,trck.ch,personal.ev.ch,personal.trck.ch,human.ev.ch,human.trck.ch,costs,vmt.by.region,vmt.by.region.trck,geners,vehs,by.r.bike,bike.ch,vmt.by.region.bike) {
	param.names <- exper$param.names
	run.params <- copy(exper$runs)[,run:=1:.N]
	n.days.in.run <- length(inputs$set$t)/24
	inputs <- all.inputs[[1]]
	#personal.chargers <- factor.chargers.prepped$personal.chargers
	#personal.chargers.trcks <- factor.chargers.prepped$personal.chargers.trcks
	#human.chargers <- factor.chargers.prepped$human.chargers
	#human.chargers.trcks <- factor.chargers.prepped$human.chargers.trcks
	#personal.evs <- factor.fleets.prepped$personal.evs
	#personal.trcks <- factor.fleets.prepped$personal.trcks
	#human.evs <- factor.fleets.prepped$human.evs
	#human.trcks <- factor.fleets.prepped$human.trcks

	factor.fleets.prepped <- prepData.factor.fleets(param.names,run.params,all.inputs,by.r,by.r.trck,by.r.bike,res)
	...
	factor.chargers.prepped <- prepData.factor.chargers(param.names,run.params,all.inputs,by.r,by.r.trck,by.r.bike)
	factor.charging.prepped <- prepData.factor.charging(param.names,run.params,veh.ch,trck.ch,bike.ch,personal.ev.ch,personal.trck.ch,human.ev.ch,human.trck.ch,inputs)
	factor.costs.prepped <- prepData.factor.costs(param.names,run.params,inputs,factor.chargers.prepped$personal.chargers,factor.chargers.prepped$personal.chargers.trcks,factor.chargers.prepped$human.chargers,factor.chargers.prepped$human.chargers.trcks,costs,factor.fleets.prepped$personal.evs,factor.fleets.prepped$personal.trcks,factor.fleets.prepped$human.evs,factor.fleets.prepped$human.trcks,vmt.by.region,n.days.in.run)
	factor.emissions.prepped <- prepData.factor.emissions(param.names,run.params,vmt.by.region,vmt.by.region.trck,vmt.by.region.bike,res,geners,factor.fleets.prepped$to.plot.fleet,n.days.in.run)

	metric.units <- data.table(metric=c('Fleet Size','# Chargers','Peak Load','Emissions','Emissions_per_Passenger_Mile','Emissions_per_Capita','Cost','Cost_per_Passenger_Mile','Cost_per_Capita','SAEV_Utilization','HDV_Utilization','bike_Utilization','Person_Trips_per_SAEV'),
		scale.factor=c(1e6,1e6,1,1e6,1e-6,1,1e9,1,1,1,1,1,1),
		label=c('Millions of Vehicles','Millions of Chargers','Peak Load (GW)','Emissions (Million Tonnes CO2eq)','Emissions (grams CO2eq per Passenger-Mile)','Emissions (Tonnes CO2eq per Capita)','Cost (Billions of $)','Cost ($ per Passenger-Mile)','Cost ($ per Capita)','Vehicle-Hours per Day','Vehicle-Hours per Day','Vehicle-Hours per Day','Person-trips per SAEV per Day'))
...
prepData.factor.fleets <- function(param.names,run.params,all.inputs,by.r,by.r.trck,by.r.bike,res) {
	personal.ev.conversion.eff <- 0.325
	personal.evs <- rbindlist(lapply(1:length(all.inputs),function(i){ all.inputs[[i]]$parameters$personalEVFleetSize[,.(value=sum(value),run=i),by='type'] }))[,.(run,b=pp("Private_auto_",type),value)]
	...
	personal.trck.conversion.eff <- 0.325
	personal.trcks <- rbindlist(lapply(1:length(all.inputs),function(i){ all.inputs[[i]]$parameters$truckpersonalEVFleetSize[,.(value=sum(value),run=i),by='type'] }))[,.(run,tb=pp("Private_trck_auto_",type),value)]
	personal.trcks <- join.on(personal.trcks,run.params,'run','run')
	personal.trcks[,batt.kwh:=unlist(lapply(str_split(tb,"EV"),function(ll){ as.numeric(ll[2])}))*personal.trck.conversion.eff]

	human.ev.conversion.eff <- 0.325
	human.evs <- rbindlist(lapply(1:length(all.inputs),function(i){ all.inputs[[i]]$parameters$humanEVFleetSize[,.(value=sum(value),run=i),by='type'] }))[,.(run,b=pp("Private__human_",type),value)]
	human.evs <- join.on(human.evs,run.params,'run','run')
	human.evs[,batt.kwh:=unlist(lapply(str_split(b,"EV"),function(ll){ as.numeric(ll[2])}))*human.ev.conversion.eff]
	
	human.trck.conversion.eff <- 0.325
	human.trcks <- rbindlist(lapply(1:length(all.inputs),function(i){ all.inputs[[i]]$parameters$truckhumanEVFleetSize[,.(value=sum(value),run=i),by='type'] }))[,.(run,tb=pp("Private_trck_human_",type),value)]
	human.trcks <- join.on(human.trcks,run.params,'run','run')
	human.trcks[,batt.kwh:=unlist(lapply(str_split(tb,"EV"),function(ll){ as.numeric(ll[2])}))*human.trck.conversion.eff]
	
	to.plot.fleet <- merge(x=by.r,y=run.params,by='run')
	to.plot.fleet <- rbindlist(list(to.plot.fleet[group=='Fleet',.(value=sum(fleetSize),bplus=pp('SAEV_BEV',substr(b,2,nchar(b)))),by=c('run',param.names,'b')],personal.evs,human.evs),fill=T)
	not.needed <- to.plot.fleet[,sum(value),by='b'][V1<=1]$b
	to.plot.fleet <- to.plot.fleet[!b%in%not.needed]
	to.plot.fleet[,variable:=ifelse(is.na(bplus),b,bplus)]
	to.plot.fleet[,metric:='Fleet Size']
	...
	to.plot.truckfleet <- rbindlist(list(to.plot.truckfleet[group=='Fleet',.(value=sum(truckfleetSize),bplus=pp('HDV_BEV',substr(tb,3,nchar(tb)))),by=c('run',param.names,'tb')],personal.trcks,human.trcks),fill=T)
	...
	to.plot.bikefleet <- merge(x=by.r.bike,y=run.params,by='run')
	to.plot.bikefleet <- to.plot.bikefleet[group=='Fleet',.(value=sum(bikefleetSize),bplus=pp('Ebike_BEV',substr(bb,3,nchar(bb)))),by=c('run',param.names,'bb')]
	names(to.plot.bikefleet)[names(to.plot.bikefleet)=='bb'] <- 'b'
	not.needed <- to.plot.bikefleet[,sum(value),by='b'][V1<=1]$b
	to.plot.bikefleet <- to.plot.bikefleet[!b%in%not.needed]
	to.plot.bikefleet[,variable:=ifelse(is.na(bplus),b,bplus)]
	to.plot.bikefleet[,metric:='Fleet Size']

	to.plot.fleet <- rbindlist(list(to.plot.fleet,to.plot.truckfleet,to.plot.bikefleet),fill=T)
	to.plot.fleet[,col:=getPalette(b)[match(to.plot.fleet$b,u(to.plot.fleet$b))]]
	to.plot.fleet <- join.on(to.plot.fleet,res[['b']],c('run','b'),c('run','b'))
	to.plot.fleet <- join.on(to.plot.fleet,res[['tb']],c('run','b'),c('run','tb'))
	to.plot.fleet <- join.on(to.plot.fleet,res[['bb']],c('run','b'),c('run','bb'))
	to.plot.fleet[is.na(batteryCapacity)&grepl('Private',b,fixed=TRUE),':='(batteryCapacity=batt.kwh,conversionEfficiency=personal.ev.conversion.eff)]
	to.plot.fleet[is.na(batteryCapacity)&grepl('Private_trck',b,fixed=TRUE),':='(batteryCapacity=batt.kwh,conversionEfficiency=personal.trck.ev.conversion.eff)]
	...
	to.plot.fleet[is.na(batteryCapacity)&grepl('bb',b,fixed=TRUE),':='(batteryCapacity=bikebatteryCapacity,conversionEfficiency=bikeconversionEfficiency)]
	return(list('personal.evs'=personal.evs,'personal.trcks'=personal.trcks,'human.evs'=human.evs,'human.trcks'=human.trcks,'to.plot.fleet'=to.plot.fleet))
}
...
prepData.factor.chargers <- function(param.names,run.params,all.inputs,by.r,by.r.trck,by.r.bike) {	
	...
	personal.chargers.trcks <- rbindlist(lapply(1:length(all.inputs),function(i){ all.inputs[[i]]$parameters$truckpersonalEVChargers[,.(value=sum(value),run=i),by=c('type','level')] }))[,.(run,type,level,value)]
	personal.chargers.trcks[,variable:=pp(type,'Ptrck_Chgr')]
	personal.chargers.trcks[,level.kw:=ifelse(level=='L1',20,ifelse(level=='L2',50,100))]
	personal.chargers.trcks <- join.on(personal.chargers.trcks,run.params,'run','run')
	
	human.chargers <- rbindlist(lapply(1:length(all.inputs),function(i){ all.inputs[[i]]$parameters$humanEVChargers[,.(value=sum(value),run=i),by=c('type','level')] }))[,.(run,type,level,value)]
	human.chargers[,variable:=pp(type,'_Chgr_h')]
	human.chargers[,level.kw:=ifelse(level=='L1',1.5,ifelse(level=='L2',6.7,50))]
	human.chargers <- join.on(human.chargers,run.params,'run','run')
	
	human.chargers.trcks <- rbindlist(lapply(1:length(all.inputs),function(i){ all.inputs[[i]]$parameters$truckhumanEVChargers[,.(value=sum(value),run=i),by=c('type','level')] }))[,.(run,type,level,value)]
	human.chargers.trcks[,variable:=pp(type,'Ptrck_Chgr_h')]
	human.chargers.trcks[,level.kw:=ifelse(level=='L1',20,ifelse(level=='L2',50,100))]
	human.chargers.trcks <- join.on(human.chargers.trcks,run.params,'run','run')
	...
	to.plot.chargers.bikes <- merge(x=by.r.bike,y=run.params,by='run')
	to.plot.chargers.bikes <- to.plot.chargers.bikes[group=='Chargers',.(value=sum(bikenumChargers)),by=c('run',param.names,'bl')]
	to.plot.chargers.bikes[,level.kw:=as.numeric(substr(bl,3,nchar(bl)))]
	to.plot.chargers.bikes[,variable:=pp('bike_Chgr_',ifelse(level.kw<=20,'AC','DC'))]

	to.plot.chargers <- rbindlist(list(to.plot.chargers,to.plot.chargers.trcks,to.plot.chargers.bikes,personal.chargers,personal.chargers.trcks,human.chargers,human.chargers.trcks),fill=T)
	to.plot.chargers[,l:=ifelse(grepl('HDV',variable,fixed=TRUE),paste0('HDV',str_pad(trunc(level.kw),4,pad='0')),l)]
	to.plot.chargers[,l:=ifelse(grepl('bike',variable,fixed=TRUE),paste0('bike',str_pad(trunc(level.kw),4,pad='0')),l)]
	to.plot.chargers <- to.plot.chargers[,.(value=sum(value),l=l[1]),by=c('run','variable',param.names)]
	to.plot.chargers[,metric:='# Chargers']
	to.plot.chargers[is.na(l),l:=variable]
	to.plot.chargers[,l.ordered:=ifelse(substr(l,1,1)=='L',pp('a',l),ifelse(substr(l,1,1)=='H',pp('b',l),ifelse(substr(l,1,1)=='b',pp('c',l),l)))]
	to.plot.chargers[,col:=getPalette(l)[match(l.ordered,u(l.ordered))]]

	return(list('personal.chargers'=personal.chargers,'personal.chargers.trcks'=personal.chargers.trcks,'human.chargers'=human.chargers,'human.chargers.trcks'=human.chargers.trcks,'to.plot.chargers'=to.plot.chargers))
}

prepData.factor.charging <- function(param.names,run.params,veh.ch,trck.ch,bike.ch,personal.ev.ch,personal.trck.ch,human.ev.ch,human.trck.ch,inputs) {
	to.plot.ch.all <- rbindlist(list(veh.ch,trck.ch,bike.ch,personal.ev.ch,personal.trck.ch,human.ev.ch,human.trck.ch),fill=T,use.names=T)
	to.plot.ch.all <- merge(x=to.plot.ch.all,y=run.params,by='run')
	to.plot.ch.all <- join.on(to.plot.ch.all,to.plot.ch.all[,.(gw=sum(gw.charging,na.rm=T)),by=c('run','t')][,.(peak.t=t[which.max(gw)]),by='run'],'run','run')
	to.plot.ch.all[is.na(l),l:=tl]
	to.plot.ch.all[is.na(l),l:=bl]
	
	to.plot.ch1 <- rbindlist(list(veh.ch,bike.ch,personal.ev.ch,human.ev.ch),fill=T,use.names=T)
	to.plot.ch1 <- merge(x=to.plot.ch1,y=run.params,by='run')
	to.plot.ch1 <- join.on(to.plot.ch1,to.plot.ch1[,.(gw=sum(gw.charging,na.rm=T)),by=c('run','t')][,.(peak.t=t[which.max(gw)]),by='run'],'run','run')
	to.plot.ch1[is.na(l),l:=bl]
	
	to.plot.ch2 <- rbindlist(list(trck.ch,personal.trck.ch,human.trck.ch),fill=T,use.names=T)
	to.plot.ch2 <- merge(x=to.plot.ch2,y=run.params,by='run')
	to.plot.ch2 <- join.on(to.plot.ch2,to.plot.ch2[,.(gw=sum(gw.charging,na.rm=T)),by=c('run','t')][,.(peak.t=t[which.max(gw)]),by='run'],'run','run')

	to.plot.peak.ch <- to.plot.ch.all[t==peak.t,.(value=sum(gw.charging,na.rm=T)),by=c('run','l',param.names)]
	to.plot.peak.ch[,level.kw:=ifelse(grepl('tL',l,fixed=TRUE),as.numeric(substr(l,3,nchar(l))),ifelse(grepl('bL',l,fixed=TRUE),as.numeric(substr(l,3,nchar(l))),as.numeric(substr(l,2,nchar(l)))))]
	to.plot.peak.ch[,variable:=ifelse(l=='Private automated EVs','Private_auto_EV_Load',ifelse(l =='Private automated electric trucks','Private_auto_Truck_Load',ifelse(l=='Private human-driven EVs','Private_human_driven_EV_Load',ifelse(l=='Private human-driven electric trucks','Private_human_driven_Truck_Load',ifelse(grepl('tL',l,fixed=TRUE),'HDV_Load_DC',ifelse(grepl('bL',l,fixed=TRUE),'bike_Load_DC',pp('SAEV_Load_',ifelse(level.kw<=20,'AC','DC'))))))))]
	to.plot.peak.ch <- to.plot.peak.ch[,.(value=sum(value),l=l[1],level.kw=level.kw[1]),by=c('run',param.names,'variable')]
	to.plot.peak.ch[,metric:='Peak Load']
	to.plot.peak.ch[,l.ordered:=ifelse(substr(l,1,1)=='L',pp('a',l),ifelse(substr(l,1,1)=='t',pp('b',l),ifelse(substr(l,1,1)=='b',pp('c',l),l)))]
	to.plot.peak.ch[,col:=getPalette(l)[match(l.ordered,u(l.ordered))]]

	to.plot.ch.agg <- to.plot.ch1[,.(gwh=sum(gw.charging,na.rm=T)),by=c('run',param.names,'charger.level','l','t')]
	to.plot.ch.agg.trck <- to.plot.ch2[,.(gwh=sum(gw.charging,na.rm=T)),by=c('run',param.names,'charger.level','tl','t')]
	to.plot.ch.agg <- disag.the.private.load(to.plot.ch.agg,inputs$parameters$personalEVUnmanagedLoads,inputs$parameters$humanEVUnmanagedLoads)
	to.plot.ch.agg.trck <- disag.the.private.trck.load(to.plot.ch.agg.trck,inputs$parameters$truckpersonalEVUnmanagedLoads,inputs$parameters$truckhumanEVUnmanagedLoads)
	to.plot.ch.agg <- rbindlist(list(to.plot.ch.agg,to.plot.ch.agg.trck),fill=T)
	to.plot.ch.agg[is.na(l),l:=tl]
	to.plot.ch.agg[,l.ordered:=ifelse(substr(l,1,1)=='L',pp('a',l),ifelse(substr(l,1,1)=='t',pp('b',l),l))]
	to.plot.ch.agg[,col:=getPalette(l)[match(l.ordered,u(l.ordered))]]
	the.ch.cols <- to.plot.ch.agg$col
	names(the.ch.cols) <- to.plot.ch.agg$charger.level

	return(list('to.plot.ch'=to.plot.ch.all,'to.plot.peak.ch'=to.plot.peak.ch,'to.plot.ch.agg'=to.plot.ch.agg,'the.ch.cols'=the.ch.cols))
}

prepData.factor.costs <- function(param.names,run.params,inputs,personal.chargers,personal.chargers.trcks,human.chargers,human.chargers.trcks,costs,personal.evs,personal.trcks,human.evs,human.trcks,vmt.by.region,n.days.in.run) {
	...
	trckCapCost <- 10e4
	trckPerDayCost <- 1000 / 365
	trckPerMileCosts <- 0.04
	private.charger.levels <- u(personal.chargers$level.kw)
	private.trck.charger.levels <- u(personal.chargers.trcks$level.kw)
	...
	infra.cost <- infra.cost[,.(privateAutoInfrastructureCost=(sum(cost)*dailyDiscount*(1+dailyDiscount)^privateChargerLifetime)/((1+dailyDiscount)^privateChargerLifetime-1)),by='run']
	infra.cost.human <- infra.cost
	colnames(infra.cost.human)[2]<-"privateHumanInfrastructureCost"
	...
	veh.cost <- personal.evs[,.(n=sum(value)),by=c('run','batt.kwh')][,.(privateAutoFleetCost=sum(n*(vehPerDayCost + vehCapCost*veh.amort.ratio + batt.kwh*inputs$parameters$batteryCapitalCost$value*batt.amort.ratio + vehPerMileCosts*12e3/365))),by='run']
	veh.cost.human <- human.evs[,.(n=sum(value)),by=c('run','batt.kwh')][,.(privateHumanFleetCost=sum(n*(vehPerDayCost + vehCapCost*veh.amort.ratio + batt.kwh*inputs$parameters$batteryCapitalCost$value*batt.amort.ratio + vehPerMileCosts*12e3/365))),by='run']
	
	infra.cost.trck <- join.on(personal.chargers.trcks,data.table(level.kw=private.trck.charger.levels,cost.per.kw=l10ChargerCost + c(private.trck.charger.levels-10)*chargerCostSuperlinear),'level.kw','level.kw')
	infra.cost.trck[level=='L1',cost.per.kw:=50]
	infra.cost.trck[,cost:=cost.per.kw*level.kw*value]
	infra.cost.trck <- infra.cost.trck[,.(privateAutoTruckInfrastructureCost=0.5*(sum(cost)*dailyDiscount*(1+dailyDiscount)^privateChargerLifetime)/((1+dailyDiscount)^privateChargerLifetime-1)),by='run']
	infra.cost.trck.human <- infra.cost.trck
	colnames(infra.cost.trck.human)[2]<-"privateHumanTruckInfrastructureCost"
	veh.amort.ratio.trck <- dailyDiscount*(1+dailyDiscount)^privateVehicleLifetime / ((1+dailyDiscount)^privateVehicleLifetime - 1)
	batt.amort.ratio.trck <- dailyDiscount*(1+dailyDiscount)^privateBatteryLifetime / ((1+dailyDiscount)^privateBatteryLifetime - 1)
	veh.cost.trck <- personal.trcks[,.(n=sum(value)),by=c('run','batt.kwh')][,.(privateAutoTruckFleetCost=0.4*sum(n*(trckPerDayCost + trckCapCost*veh.amort.ratio + batt.kwh*inputs$parameters$truckbatteryCapitalCost$value*batt.amort.ratio.trck + trckPerMileCosts*12e3/365))),by='run']
	veh.cost.trck.human <- human.trcks[,.(n=sum(value)),by=c('run','batt.kwh')][,.(privateHumanTruckFleetCost=0.4*sum(n*(trckPerDayCost + trckCapCost*veh.amort.ratio + batt.kwh*inputs$parameters$truckbatteryCapitalCost$value*batt.amort.ratio.trck + trckPerMileCosts*12e3/365))),by='run']
	...
	costs[,totalBikeFleetCost:=bikevehicleMaintCost/n.days.in.run+bikefleetCost]
	costs[,totalEnergyCost:=demandChargeCost/n.days.in.run+energyCost/n.days.in.run]
	to.plot.cost <- merge(x=costs,y=run.params,by='run')
	to.plot.cost <- data.table(melt(to.plot.cost,measure.vars=c('totalEnergyCost','totalFleetCost','totalTruckFleetCost','totalBikeFleetCost','infrastructureCost','truckinfrastructureCost','bikeinfrastructureCost'),id.vars=c('run',param.names)))[,.(value=sum(value)),by=c('variable','run',param.names)]
	to.plot.cost <- rbindlist(list(to.plot.cost,melt(join.on(join.on(infra.cost,veh.cost,'run','run'),run.params,'run','run'),measure.vars=c('privateAutoInfrastructureCost','privateAutoFleetCost'),id.vars=c('run',param.names))),fill=T)
	to.plot.cost <- rbindlist(list(to.plot.cost,melt(join.on(join.on(infra.cost.trck,veh.cost.trck,'run','run'),run.params,'run','run'),measure.vars=c('privateAutoTruckInfrastructureCost','privateAutoTruckFleetCost'),id.vars=c('run',param.names))),fill=T)

	to.plot.cost <- rbindlist(list(to.plot.cost,melt(join.on(join.on(infra.cost.human,veh.cost.human,'run','run'),run.params,'run','run'),measure.vars=c('privateHumanInfrastructureCost','privateHumanFleetCost'),id.vars=c('run',param.names))),fill=T)
	to.plot.cost <- rbindlist(list(to.plot.cost,melt(join.on(join.on(infra.cost.trck.human,veh.cost.trck.human,'run','run'),run.params,'run','run'),measure.vars=c('privateHumanTruckInfrastructureCost','privateHumanTruckFleetCost'),id.vars=c('run',param.names))),fill=T)
	...
	to.plot.cost[,variable.short:=variable]
	to.plot.cost[,value:=value*weekday.to.year.factor]
	cost.key <- data.table(variable=c('infrastructureCost','truckinfrastructureCost','bikeinfrastructureCost','totalFleetCost','totalTruckFleetCost','totalBikeFleetCost','totalEnergyCost','privateAutoInfrastructureCost','privateAutoFleetCost','privateAutoTruckInfrastructureCost','privateAutoTruckFleetCost','privateHumanInfrastructureCost','privateHumanFleetCost','privateHumanTruckInfrastructureCost','privateHumanTruckFleetCost'),cost.key=c('SAEV Infrastructure','HDV Infrastructure','Bike Infrastructure','SAEV Fleet','HDV Fleet','Bike Fleet','Energy','Private Auto Infrastructure','Private Auto Fleet','Private Auto Truck Infrastructure','Private Auto Truck Fleet','Private human-driven Infrastructure','Private human-driven Fleet','Private human-driven Truck Infrastructure','Private human-driven Truck Fleet'))
	to.plot.cost <- join.on(to.plot.cost,cost.key,'variable.short','variable')
	to.plot.cost[,variable:=pp('Cost: ',cost.key)]
	to.plot.cost[,col:=getPalette(variable.short)[match(variable.short,u(variable.short))]]
	to.plot.cost.per.pass.mile <- join.on(copy(to.plot.cost),vmt.by.region[,.(daily.pmt=sum(daily.pmt)),by='run'],'run','run')
...
prepData.factor.emissions <- function(param.names,run.params,vmt.by.region,vmt.by.region.trck,vmt.by.region.bike,res,geners,to.plot.fleet,n.days.in.run) {
	dailyDiscount <- discountRate/365
	...
	bike.lifetimes <- vmt.by.region.bike[,.(vehicleLifetime=bikevehicleLifetime[1],batteryLifetime=bikebatteryLifetime[1]),by=c('run','bb')]
	bike.lifetimes[,veh.amort.ratio:=dailyDiscount*(1+dailyDiscount)^(vehicleLifetime*365) / ((1+dailyDiscount)^(vehicleLifetime*365) - 1)]
	bike.lifetimes[,batt.amort.ratio:=dailyDiscount*(1+dailyDiscount)^(batteryLifetime*365) / ((1+dailyDiscount)^(batteryLifetime*365) - 1)]
	names(bike.lifetimes)[names(bike.lifetimes)=='bb'] <- 'b'

	all.lifetimes <- rbind(saev.lifetimes,hdv.lifetimes,bike.lifetimes)
...
disag.the.private.load <- function(df,personalEVUnmanagedLoads,humanEVUnmanagedLoads){
	df[(l!='Private automated EVs'&l!='Private human-driven EVs')&substr(l,1,1)=='L',charger.level:=pp('SAEV: ',charger.level)]
	# Split private EV out by charger level
	proportional.split <- personalEVUnmanagedLoads[,.(frac=sum(value)/max(1,sum(personalEVUnmanagedLoads$value))),by='l']
	proportional.split.human <- humanEVUnmanagedLoads[,.(frac=sum(value)/max(1,sum(humanEVUnmanagedLoads$value))),by='l']
	private.disag1 <- join.on(rbindlist(list(df[l=='Private automated EVs'][,ll:='L1'],df[l=='Private automated EVs'][,ll:='L2'],df[l=='Private automated EVs'][,ll:='L3'])),proportional.split,'ll','l')
	private.disag1[,gwh:=frac*gwh]
	private.disag1[,charger.level:=ifelse(ll=='L1','Private auto: 1.5kW',ifelse(ll=='L2','Private auto: 7kW','Private auto: 50kW'))]
	private.disag1[,l:=ifelse(ll=='L1','Private auto: 1.5kW',ifelse(ll=='L2','Private auto: 7kW','Private auto: 50kW'))]
	private.disag2 <- join.on(rbindlist(list(df[l=='Private human-driven EVs'][,ll:='L1'],df[l=='Private human-driven EVs'][,ll:='L2'],df[l=='Private human-driven EVs'][,ll:='L3'])),proportional.split.human,'ll','l')
	private.disag2[,gwh:=frac*gwh]
	private.disag2[,charger.level:=ifelse(ll=='L1','Private human-driven: 1.5kW',ifelse(ll=='L2','Private human-driven: 7kW','Private human-driven: 50kW'))]
	private.disag2[,l:=ifelse(ll=='L1','Private human-driven: 1.5kW',ifelse(ll=='L2','Private human-driven: 7kW','Private human-driven: 50kW'))]
	rbindlist(list(df[l!='Private automated EVs'&l!='Private human-driven EVs'],private.disag1,private.disag2),fill=T)
}

disag.the.private.trck.load <- function(df,truckpersonalEVUnmanagedLoads,truckhumanEVUnmanagedLoads){
  df[(tl!='Private automated electric trucks'&tl!='Private human-driven electric trucks')&substr(tl,1,1)=='t',charger.level:=pp('HDV: ',charger.level)]
  # Split private EV out by charger level
  proportional.split <- truckpersonalEVUnmanagedLoads[,.(frac=sum(value)/max(1,sum(truckpersonalEVUnmanagedLoads$value))),by='l']
  colnames(proportional.split)[1] = 'tl'
  proportional.split.human <- truckhumanEVUnmanagedLoads[,.(frac=sum(value)/max(1,sum(truckhumanEVUnmanagedLoads$value))),by='l']
  colnames(proportional.split.human)[1] = 'tl'
  private.disag1 <- join.on(rbindlist(list(df[tl=='Private automated electric trucks'][,ll:='L1'],df[tl=='Private automated electric trucks'][,ll:='L2'],df[tl=='Private automated electric trucks'][,ll:='L3'])),proportional.split,'ll','tl')
  private.disag1[,gwh:=frac*gwh]
  private.disag1[,charger.level:=ifelse(ll=='L1','Private auto truck: 5kW',ifelse(ll=='L2','Private auto truck: 20kW','Privat auto truck: 100kW'))]
  private.disag1[,tl:=ifelse(ll=='L1','Private auto truck: 5kW',ifelse(ll=='L2','Private auto truck: 20kW','Private auto truck: 100kW'))]
  private.disag2 <- join.on(rbindlist(list(df[tl=='Private human-driven electric trucks'][,ll:='L1'],df[tl=='Private human-driven electric trucks'][,ll:='L2'],df[tl=='Private human-driven electric trucks'][,ll:='L3'])),proportional.split.human,'ll','tl')
  private.disag2[,gwh:=frac*gwh]
  private.disag2[,charger.level:=ifelse(ll=='L1','Private human-driven truck: 5kW',ifelse(ll=='L2','Private human-driven truck: 20kW','Privat human-driven truck: 100kW'))]
  private.disag2[,tl:=ifelse(ll=='L1','Private human-driven truck: 5kW',ifelse(ll=='L2','Private human-driven truck: 20kW','Private human-driven truck: 100kW'))]
  rbindlist(list(df[tl!='Private human-driven electric trucks'&tl!='Private automated electric trucks'],private.disag1,private.disag2),fill=T)
}

disag.the.human.load <- function(df,humanEVUnmanagedLoads){
  df[l!='Private human-driven EVs'&substr(l,1,1)=='L',charger.level:=pp('SAEV: ',charger.level)]
  # Split private EV out by charger level
  proportional.split <- humanEVUnmanagedLoads[,.(frac=sum(value)/max(1,sum(humanEVUnmanagedLoads$value))),by='l']
  private.disag <- join.on(rbindlist(list(df[l=='Private human-driven EVs'][,ll:='L1'],df[l=='Private human-driven EVs'][,ll:='L2'],df[l=='Private human-driven EVs'][,ll:='L3'])),proportional.split,'ll','l')
  private.disag[,gwh:=frac*gwh]
  private.disag[,charger.level:=ifelse(ll=='L1','Private human-driven: 1.5kW',ifelse(ll=='L2','Private human-driven: 7kW','Private human-driven: 50kW'))]
  private.disag[,l:=ifelse(ll=='L1','Private human-driven: 1.5kW',ifelse(ll=='L2','Private human-driven: 7kW','Private human-driven: 50kW'))]
  rbindlist(list(df[l!='Private human-driven EVs'],private.disag),fill=T)
}

disag.the.human.trck.load <- function(df,truckhumanEVUnmanagedLoads){
  df[tl!='Private human-driven electric trucks'&substr(tl,1,1)=='t',charger.level:=pp('HDV: ',charger.level)]
  # Split private EV out by charger level
  proportional.split <- truckhumanEVUnmanagedLoads[,.(frac=sum(value)/max(1,sum(truckhumanEVUnmanagedLoads$value))),by='l']
  colnames(proportional.split)[1] = 'tl'
  private.disag <- join.on(rbindlist(list(df[tl=='Private human-driven electric trucks'][,ll:='L1'],df[tl=='Private human-driven electric trucks'][,ll:='L2'],df[tl=='Private human-driven electric trucks'][,ll:='L3'])),proportional.split,'ll','tl')
  private.disag[,gwh:=frac*gwh]
  private.disag[,charger.level:=ifelse(ll=='L1','Private human-driven truck: 5kW',ifelse(ll=='L2','Private human-driven truck: 20kW','Private human-driven truck: 100kW'))]
  private.disag[,tl:=ifelse(ll=='L1','Private human-driven truck: 5kW',ifelse(ll=='L2','Private human-driven truck: 20kW','Private human-driven truck: 100kW'))]
  rbindlist(list(df[tl!='Private human-driven electric trucks'],private.disag),fill=T)
}
...
plot.lightduty.all <- function(run.i,plots.dir,vehs,exper,all.inputs,veh.ch,personal.ev.ch,human.ev.ch,en,by.r,day.axis.breaks) {
	plot.lightduty.numberOfVehicles(run.i,plots.dir,vehs,day.axis.breaks)
	plot.lightduty.charging(run.i,plots.dir,exper,all.inputs,veh.ch,personal.ev.ch,human.ev.ch,day.axis.breaks)
	plot.lightduty.personalVehs(run.i,plots.dir,all.inputs,personal.ev.ch,human.ev.ch,veh.ch,day.axis.breaks)
...
plot.lightduty.charging <- function(run.i,plots.dir,exper,all.inputs,veh.ch,personal.ev.ch,human.ev.ch,day.axis.breaks) {
	...
	to.plot <- data.table(rbindlist(list(veh.ch,personal.ev.ch,human.ev.ch),fill=T,use.names=T)[run==run.i])
	to.plot[,gwh:=gw.charging]
	to.plot[,ll:='']
	df<-to.plot
	personalEVUnmanagedLoads<-all.inputs[[run.i]]$parameters$personalEVUnmanagedLoads
	humanEVUnmanagedLoads<-all.inputs[[run.i]]$parameters$humanEVUnmanagedLoads
	to.plot <- disag.the.private.load(to.plot,all.inputs[[run.i]]$parameters$personalEVUnmanagedLoads,all.inputs[[run.i]]$parameters$humanEVUnmanagedLoads)
...
plot.lightduty.personalVehs <- function(run.i,plots.dir,all.inputs,personal.ev.ch,human.ev.ch,veh.ch,day.axis.breaks) {
	...
	to.plot <- rbindlist(list(veh.ch,human.ev.ch),fill=T,use.names=T)[run==run.i]
	to.plot[,gwh:=gw.charging]
	to.plot[,ll:='']
	to.plot <- disag.the.private.load(to.plot,all.inputs[[run.i]]$parameters$personalEVUnmanagedLoads,all.inputs[[run.i]]$parameters$humanEVUnmanagedLoads)
	to.plot[,l.ordered:=ifelse(substr(l,1,1)=='L',pp('a',l),l)]
	...
	human.evs.total <- all.inputs[[run.i]]$parameters$humanEVFleetSize
	human.evs.total <- human.evs.total[rep(1:nrow(human.evs.total),length(u(human.ev.ch$t)))]
	human.evs.total[,t:=rep(1:length(u(human.ev.ch$t)),each=nrow(human.evs.total)/length(u(human.ev.ch$t)))]
	human.evs.total <- human.evs.total[,.(Total=sum(value)),by=.(t,rmob)]
	...
	chts.hours[,Probability:=Count/sum(Count)]
	chts.hours[,Hour:=as.numeric(Hour)+1]
	chts.hours <- chts.hours[rep(1:nrow(chts.hours),length(u(human.ev.ch$t))/24),]
	chts.hours[,t:=(1:length(u(human.ev.ch$t)))]
	chts.hours <- chts.hours[,.(t,Probability)]
	
	human.evs.total <- merge(x=human.evs.total,y=chts.hours,by='t',all.x=TRUE)
	human.evs.total[,Moving:=Total*Probability]
	
	human.evs.ch.total <- to.plot[ll%in%c('L1','L2','L3'),.(gw.charging=sum(gw.charging)),by=.(ll,t,rmob)]
	human.evs.ch.total[,Charging:=ifelse(ll=='L1',gw.charging*10^6/1.5,ifelse(ll=='L2',gw.charging*10^6/6.7,gw.charging*10^6/50))]
	human.evs.ch.total <- human.evs.ch.total[,.(Charging=sum(Charging)),by=.(t,rmob)]
	
	human.evs.total <- merge(x=human.evs.total,y=human.evs.ch.total,by=c('t','rmob'))
	human.evs.total[,Charging:=ifelse(Charging>Total,Total,Charging)]
	human.evs.total[,Moving:=ifelse(Moving+Charging>Total,Total-Charging,Moving)]
	human.evs.total[,Idle:=Total-Moving-Charging]
	human.evs.total <- human.evs.total[,.(Moving=sum(Moving),Charging=sum(Charging),Idle=sum(Idle)),by=.(t)]
	human.evs.total[Idle<0,Idle:=0]
	human.evs.total <- data.table(melt(human.evs.total,id='t',variable.name='Activity',value.name='Cars'))
	names(human.evs.total) <- c('t','Activity','Cars')
	human.evs.total$Activity <- factor(human.evs.total$Activity,levels=c('Idle','Moving','Charging'))
	
	p<-ggplot(human.evs.total,aes(x=t,y=Cars/1000,fill=Activity))+
	  geom_area()+
	  xlab('Hour')+
	  ylab('Number of vehicles (thousands)')+
	  scale_fill_manual(name='human Vehicle activity',values=c('Idle'='#bdbdbd','Moving'='#3182bd','Charging'='#ee3a3a'))+
	  scale_x_continuous(breaks=day.axis.breaks)+
	  theme_bw()
	pdf.scale <- 1
	ggsave(pp(plots.dir,'/run-',run.i,'/_ld_num-vehs-simple-human.pdf'),p,width=10*pdf.scale,height=5*pdf.scale,units='in')
}
...
plot.heavyduty.all <- function(run.i,plots.dir,vehs,exper,all.inputs,veh.ch,personal.ev.ch,human.ev.ch,en,by.r,day.axis.breaks) {
	plot.heavyduty.numberOfVehicles(run.i,plots.dir,vehs,day.axis.breaks)
  
  plot.heavyduty.charging(run.i,plots.dir,exper,all.inputs,veh.ch,personal.ev.ch,human.ev.ch,day.axis.breaks)
  
  plot.heavyduty.personalVehs(run.i,plots.dir,all.inputs,personal.ev.ch,human.ev.ch,veh.ch,day.axis.breaks)
...
plot.heavyduty.charging <- function(run.i,plots.dir,exper,all.inputs,veh.ch,personal.ev.ch,human.ev.ch,day.axis.breaks) {
  p <- ggplot(veh.ch[run==run.i,.(gw.charging=sum(gw.charging)),by=.(t,charger.level,rmob)],aes(x=t,y=gw.charging,fill=fct_rev(charger.level)))+
    geom_area()+
    facet_wrap(~rmob,scales='free_y')+
    scale_fill_manual(values = getPalette(veh.ch$charger.level),guide=guide_legend(reverse=F))+
  ...
  to.plot <- disag.the.private.trck.load(to.plot,all.inputs[[run.i]]$parameters$truckpersonalEVUnmanagedLoads,all.inputs[[run.i]]$parameters$truckhumanEVUnmanagedLoads)
  
  to.plot[,l.ordered:=ifelse(substr(tl,1,1)=='t',pp('a',tl),tl)]
  to.plot[,col:=getPalette(tl)[match(l.ordered,u(l.ordered))]]
  the.ch.cols <- to.plot$col
  names(the.ch.cols) <- to.plot$charger.level
  ...
  p <- ggplot(to.plot[,.(gwh=sum(gwh)),by=c('t','charger.level','rmob')],aes(x=t,y=gwh,fill=charger.level))+
    ...
    theme_bw()
  pdf.scale <- 1
  ggsave(pp(plots.dir,'/run-',run.i,'/_hd_charging-all.pdf'),p,width=10*pdf.scale,height=2*pdf.scale,units='in')
  ...
  p <- ggplot(to.plot[,.(gwh=sum(gwh)),by=c('t','charger.level')],aes(x=t,y=gwh,fill=charger.level))+
    ...
    theme_bw()
  pdf.scale <- 1
  ggsave(pp(plots.dir,'/run-',run.i,'/_hd_charging-all-agg.pdf'),p,width=10*pdf.scale,height=5*pdf.scale,units='in')
}

plot.heavyduty.personalVehs <- function(run.i,plots.dir,all.inputs,personal.ev.ch,human.ev.ch,veh.ch,day.axis.breaks) {
  to.plot <- rbindlist(list(veh.ch,personal.ev.ch),fill=T,use.names=T)[run==run.i]
  to.plot[,gwh:=gw.charging]
  to.plot[,ll:='']
  ...
  personal.evs.total <- all.inputs[[run.i]]$parameters$truckpersonalEVFleetSize
  personal.evs.total <- personal.evs.total[rep(1:nrow(personal.evs.total),length(u(personal.ev.ch$t)))]
  personal.evs.total[,t:=rep(1:length(u(personal.ev.ch$t)),each=nrow(personal.evs.total)/length(u(personal.ev.ch$t)))]
  personal.evs.total <- personal.evs.total[,.(Total=sum(value)),by=.(t,rmob)]
  ...
  personal.evs.total <- merge(x=personal.evs.total,y=chts.hours,by='t',all.x=TRUE)
  personal.evs.total[,Moving:=Total*Probability*3]
  
  personal.evs.ch.total <- to.plot[ll%in%c('L1','L2','L3'),.(gw.charging=sum(gw.charging)),by=.(ll,t,rmob)]
  personal.evs.ch.total[,Charging:=ifelse(ll=='L1',gw.charging*10^6/1.5,ifelse(ll=='tL2',gw.charging*10^6/6.7,gw.charging*10^6/50))]
  personal.evs.ch.total <- personal.evs.ch.total[,.(Charging=sum(Charging)),by=.(t,rmob)]
  ...
  p<-ggplot(personal.evs.total,aes(x=t,y=Cars/1000,fill=Activity))+
    ...
    xlab('Hour')+
    ylab('Number of vehicles (thousands)')+
    scale_fill_manual(name='Personal truck activity',values=c('Idle'='#bdbdbd','Moving'='#3182bd','Charging'='#ee3a3a'))+
    scale_x_continuous(breaks=day.axis.breaks)+
    theme_bw()
  pdf.scale <- 1
  ggsave(pp(plots.dir,'/run-',run.i,'/_hd_num-vehs-simple-personal.pdf'),p,width=10*pdf.scale,height=5*pdf.scale,units='in')
  
  to.plot <- rbindlist(list(veh.ch,human.ev.ch),fill=T,use.names=T)[run==run.i]
  to.plot[,gwh:=gw.charging]
  to.plot[,ll:='']
  df<- to.plot
  truckpersonalEVUnmanagedLoads<-all.inputs[[run.i]]$parameters$truckpersonalEVUnmanagedLoads
  truckhumanEVUnmanagedLoads<-all.inputs[[run.i]]$parameters$truckhumanEVUnmanagedLoads
  to.plot <- disag.the.private.trck.load(to.plot,all.inputs[[run.i]]$parameters$truckpersonalEVUnmanagedLoads,all.inputs[[run.i]]$parameters$truckhumanEVUnmanagedLoads)
  to.plot[,l.ordered:=ifelse(substr(tl,1,1)=='t',pp('a',tl),tl)]
  to.plot[,col:=getPalette(tl)[match(l.ordered,u(l.ordered))]]
  the.ch.cols <- to.plot$col
  ...
  human.evs.total <- all.inputs[[run.i]]$parameters$truckhumanEVFleetSize
  human.evs.total <- human.evs.total[rep(1:nrow(human.evs.total),length(u(human.ev.ch$t)))]
  human.evs.total[,t:=rep(1:length(u(human.ev.ch$t)),each=nrow(human.evs.total)/length(u(human.ev.ch$t)))]
  human.evs.total <- human.evs.total[,.(Total=sum(value)),by=.(t,rmob)]
  ...
  human.evs.total <- merge(x=human.evs.total,y=chts.hours,by='t',all.x=TRUE)
  human.evs.total[,Moving:=Total*Probability*3]
  
  human.evs.ch.total <- to.plot[ll%in%c('L1','L2','L3'),.(gw.charging=sum(gw.charging)),by=.(ll,t,rmob)]
  human.evs.ch.total[,Charging:=ifelse(ll=='L1',gw.charging*10^6/1.5,ifelse(ll=='tL2',gw.charging*10^6/6.7,gw.charging*10^6/50))]
  human.evs.ch.total <- human.evs.ch.total[,.(Charging=sum(Charging)),by=.(t,rmob)]
  ...
  p<-ggplot(human.evs.total,aes(x=t,y=Cars/1000,fill=Activity))+
    ...
    xlab('Hour')+
    ylab('Number of vehicles (thousands)')+
    scale_fill_manual(name='human truck activity',values=c('Idle'='#bdbdbd','Moving'='#3182bd','Charging'='#ee3a3a'))+
    scale_x_continuous(breaks=day.axis.breaks)+
    theme_bw()
  pdf.scale <- 1
  ggsave(pp(plots.dir,'/run-',run.i,'/_hd_num-vehs-simple-human.pdf'),p,width=10*pdf.scale,height=5*pdf.scale,units='in')
}
