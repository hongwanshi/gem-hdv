...
disag.the.private.trck.load <- function(df,truckpersonalEVUnmanagedLoads){
  ...
  # Split private EV out by charger level
  proportional.split <- truckpersonalEVUnmanagedLoads[,.(frac=sum(value)/sum(truckpersonalEVUnmanagedLoads$value)),by='l']
  ...
  private.disag[,gwh:=frac*gwh]
  private.disag[,charger.level:=ifelse(ll=='L1','Private: 3kW',ifelse(ll=='L2','Private: 10kW','Private: 50kW'))]
  private.disag[,tl:=ifelse(ll=='L1','Private: 3kW',ifelse(ll=='L2','Private: 10kW','Private: 50kW'))]
