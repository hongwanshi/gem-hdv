...
prep.inputs.mobility.truck <- function(exper.row,param.names,common.inputs){
  ...
  if('truckelectrificationPenetration' %in% param.names){
  ...
  all.dem <- all.dem[,list(t,td=pp('td',td),rmob,value=value*truckfractionSAEVs*truckelectrificationPenetration*truckvmtReboundFactor)]
  ...
  inputs$parameters$truckdemandUnscaled <- all.dem1
  all.dem1 <- all.dem1[,list(t,td,rmob,value=value*truckfractionSAEVs*truckelectrificationPenetration*truckvmtReboundFactor)]
  inputs$parameters$truckdemand <- all.dem1
  ...
  inputs$parameters$trucktravelDistance <- read.csv(pp(gem.raw.inputs,'trucks/parameter_selection_traveldistance.csv'))
  ...
  #### CHARGER CAPITAL ####
  if('tl10ChargerCost'%in%param.names){
    tl10ChargerCost <- exper.row$tl10ChargerCost
  }
  if('tchargerCostSuperlinear'%in%param.names){
    tchargerCostSuperlinear <- exper.row$tchargerCostSuperlinear
  }
  truckchargerCapitalCost <- tl10ChargerCost + c(truckcharger.levels-min(truckcharger.levels))*tchargerCostSuperlinear
