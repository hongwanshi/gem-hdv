...
run.all <- function(exper,all.inputs,res,plots.dir) {
  ...
  run.i = 1
	...
	for(run.i in u(lightduty.prepped$vehs$run)) {
		...
		plot.heavyduty.all(run.i,plots.dir,heavyduty.prepped$trcks,exper,all.inputs,heavyduty.prepped$trck.ch,heavyduty.prepped$personal.trck.ch,heavyduty.prepped$en.trck,heavyduty.prepped$by.r.trck,day.axis.breaks)
...
prepData.all.heavyduty <- function(exper,inputs,res,plots.dir) {
	...
	personal.trck.ch[,tl:='Private electric trucks']
	personal.trck.ch[,charger.level:='Private electric trucks']
	...
	output <- list('trck.mv'=trck.mv,'td.dot'=td.dot,'tl.dot'=tl.dot,'trck.ch'=trck.ch,'trcks'=trcks,'personal.trck.ch'=personal.trck.ch,'en.trck'=en.trck,'batt.trck'=batt.trck,'by.r.trck'=by.r.trck,'vmt.trck'=vmt.trck,'vmt.by.region.trck'=vmt.by.region.trck)
	return(output)
}
...
prepData.factor.all <- function(all.inputs,inputs,res,by.r,by.r.trck,veh.ch,trck.ch,personal.ev.ch,personal.trck.ch,costs,vmt.by.region,vmt.by.region.trck,geners,vehs,by.r.bike,bike.ch,vmt.by.region.bike) {
	...
	factor.charging.prepped <- prepData.factor.charging(param.names,run.params,veh.ch,trck.ch,bike.ch,personal.ev.ch,personal.trck.ch,inputs)
	factor.costs.prepped <- prepData.factor.costs(param.names,run.params,inputs,factor.chargers.prepped$personal.chargers,factor.chargers.prepped$personal.chargers.trcks,costs,factor.fleets.prepped$personal.evs,factor.fleets.prepped$personal.trcks,vmt.by.region,n.days.in.run)
...
prepData.factor.fleets <- function(param.names,run.params,all.inputs,by.r,by.r.trck,by.r.bike,res) {
	...
	personal.trcks <- rbindlist(lapply(1:length(all.inputs),function(i){ all.inputs[[i]]$parameters$truckpersonalEVFleetSize[,.(value=sum(value),run=i),by='type'] }))[,.(run,tb=pp("Private_trck_",type),value)]
	...
	to.plot.truckfleet <- rbindlist(list(to.plot.truckfleet[group=='Fleet',.(value=sum(truckfleetSize),bplus=pp('HDV_BEV',substr(tb,3,nchar(tb)))),by=c('run',param.names,'tb')],personal.trcks),fill=T)
	...
	to.plot.fleet[is.na(batteryCapacity)&grepl('bb',b,fixed=TRUE),':='(batteryCapacity=bikebatteryCapacity,conversionEfficiency=bikeconversionEfficiency)]
	return(list('personal.evs'=personal.evs,'personal.trcks'=personal.trcks,'to.plot.fleet'=to.plot.fleet))
}
...
prepData.factor.chargers <- function(param.names,run.params,all.inputs,by.r,by.r.trck,by.r.bike) {	
	...
	to.plot.chargers <- rbindlist(list(to.plot.chargers,to.plot.chargers.trcks,to.plot.chargers.bikes,personal.chargers,personal.chargers.trcks),fill=T)
	...
	return(list('personal.chargers'=personal.chargers,'personal.chargers.trcks'=personal.chargers.trcks,'to.plot.chargers'=to.plot.chargers))
}

prepData.factor.charging <- function(param.names,run.params,veh.ch,trck.ch,bike.ch,personal.ev.ch,personal.trck.ch,inputs) {
	to.plot.ch.all <- rbindlist(list(veh.ch,trck.ch,bike.ch,personal.ev.ch,personal.trck.ch),fill=T,use.names=T)
	...
	to.plot.ch1 <- rbindlist(list(veh.ch,bike.ch,personal.ev.ch),fill=T,use.names=T)
	...
	to.plot.ch2 <- rbindlist(list(trck.ch,personal.trck.ch),fill=T,use.names=T)
	...
	to.plot.peak.ch[,variable:=ifelse(l=='Private EVs','Private_EV_Load',ifelse(l =='Private electric trucks','Private_Truck_Load',ifelse(grepl('tL',l,fixed=TRUE),'HDV_Load_DC',ifelse(grepl('bL',l,fixed=TRUE),'bike_Load_DC',pp('SAEV_Load_',ifelse(level.kw<=20,'AC','DC'))))))]
	to.plot.peak.ch <- to.plot.peak.ch[,.(value=sum(value),l=l[1],level.kw=level.kw[1]),by=c('run',param.names,'variable')]
	to.plot.peak.ch[,metric:='Peak Load']
	...
	to.plot.ch.agg <- disag.the.private.load(to.plot.ch.agg,inputs$parameters$personalEVUnmanagedLoads)
	to.plot.ch.agg.trck <- disag.the.private.trck.load(to.plot.ch.agg.trck,inputs$parameters$truckpersonalEVUnmanagedLoads)
...
prepData.factor.costs <- function(param.names,run.params,inputs,personal.chargers,personal.chargers.trcks,costs,personal.evs,personal.trcks,vmt.by.region,n.days.in.run) {
	...
	infra.cost.trck <- infra.cost.trck[,.(privateTruckInfrastructureCost=(sum(cost)*dailyDiscount*(1+dailyDiscount)^privateChargerLifetime)/((1+dailyDiscount)^privateChargerLifetime-1)),by='run']
	...
	veh.cost.trck <- personal.trcks[,.(n=sum(value)),by=c('run','batt.kwh')][,.(privateTruckFleetCost=sum(n*(trckPerDayCost + trckCapCost*veh.amort.ratio + batt.kwh*inputs$parameters$truckbatteryCapitalCost$value*batt.amort.ratio.trck + trckPerMileCosts*12e3/365))),by='run']
	...
	to.plot.cost <- rbindlist(list(to.plot.cost,melt(join.on(join.on(infra.cost.trck,veh.cost.trck,'run','run'),run.params,'run','run'),measure.vars=c('privateTruckInfrastructureCost','privateTruckFleetCost'),id.vars=c('run',param.names))),fill=T)
	
	to.plot.cost[,max.value:=max(to.plot.cost[,.(val=sum(value)),by='run']$val)]
	to.plot.cost[,metric:='Cost']
	to.plot.cost[,variable.short:=variable]
	to.plot.cost[,value:=value*weekday.to.year.factor]
	cost.key <- data.table(variable=c('infrastructureCost','truckinfrastructureCost','bikeinfrastructureCost','totalFleetCost','totalTruckFleetCost','totalBikeFleetCost','totalEnergyCost','privateInfrastructureCost','privateFleetCost','privateTruckInfrastructureCost','privateTruckFleetCost'),cost.key=c('SAEV Infrastructure','HDV Infrastructure','Bike Infrastructure','SAEV Fleet','HDV Fleet','Bike Fleet','Energy','Private Infrastructure','Private Fleet','Private Truck Infrastructure','Private Truck Fleet'))
	to.plot.cost <- join.on(to.plot.cost,cost.key,'variable.short','variable')
	to.plot.cost[,variable:=pp('Cost: ',cost.key)]
	to.plot.cost[,col:=getPalette(variable.short)[match(variable.short,u(variable.short))]]
	to.plot.cost.per.pass.mile <- join.on(copy(to.plot.cost),vmt.by.region[,.(daily.pmt=sum(daily.pmt)),by='run'],'run','run')
...
disag.the.private.trck.load <- function(df,truckpersonalEVUnmanagedLoads){
  df[tl!='Private electric trucks'&substr(tl,1,1)=='t',charger.level:=pp('HDV: ',charger.level)]
  # Split private EV out by charger level
  ...
  private.disag <- join.on(rbindlist(list(df[tl=='Private electric trucks'][,ll:='L1'],df[tl=='Private electric trucks'][,ll:='L2'],df[tl=='Private electric trucks'][,ll:='L3'])),proportional.split,'ll','tl')
  private.disag[,gwh:=frac*gwh]
  private.disag[,charger.level:=ifelse(ll=='L1','Private truck: 5kW',ifelse(ll=='L2','Private truck: 20kW','Private truck: 100kW'))]
  private.disag[,tl:=ifelse(ll=='L1','Private truck: 5kW',ifelse(ll=='L2','Private truck: 20kW','Private truck: 100kW'))]
  rbindlist(list(df[tl!='Private electric trucks'],private.disag),fill=T)
}
...
plot.heavyduty.all <- function(run.i,plots.dir,vehs,exper,all.inputs,veh.ch,personal.ev.ch,en,by.r,day.axis.breaks) {
	plot.heavyduty.numberOfVehicles(run.i,plots.dir,vehs,day.axis.breaks)
  
  plot.heavyduty.charging(run.i,plots.dir,exper,all.inputs,veh.ch,personal.ev.ch,day.axis.breaks)
  plot.heavyduty.personalVehs(run.i,plots.dir,all.inputs,personal.ev.ch,veh.ch,day.axis.breaks)
...
plot.heavyduty.charging <- function(run.i,plots.dir,exper,all.inputs,veh.ch,personal.ev.ch,day.axis.breaks) {
  p <- ggplot(veh.ch[run==run.i,.(gw.charging=sum(gw.charging)),by=.(t,charger.level,rmob)],aes(x=t,y=gw.charging,fill=fct_rev(charger.level)))+
    geom_area()+
    facet_wrap(~rmob,scales='free_y')+
    scale_fill_manual(values = getPalette(veh.ch$charger.level),guide=guide_legend(reverse=F))+
  ...
  to.plot[,gwh:=gw.charging]
  to.plot[,ll:='']
  to.plot <- disag.the.private.trck.load(to.plot,all.inputs[[run.i]]$parameters$truckpersonalEVUnmanagedLoads)
  ...
  p <- ggplot(to.plot[,.(gwh=sum(gwh)),by=c('t','charger.level','rmob')],aes(x=t,y=gwh,fill=charger.level))+
    ...
    theme_bw()
  pdf.scale <- 1
  ggsave(pp(plots.dir,'/run-',run.i,'/_hd_charging-all.pdf'),p,width=10*pdf.scale,height=8*pdf.scale,units='in')
...
plot.heavyduty.personalVehs <- function(run.i,plots.dir,all.inputs,personal.ev.ch,veh.ch,day.axis.breaks) {
  to.plot <- rbindlist(list(veh.ch,personal.ev.ch),fill=T,use.names=T)[run==run.i]
  to.plot[,gwh:=gw.charging]
  to.plot[,ll:='']
  to.plot <- disag.the.private.trck.load(to.plot,all.inputs[[run.i]]$parameters$truckpersonalEVUnmanagedLoads)
